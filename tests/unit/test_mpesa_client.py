...
def test_b2p_service_instance(client):
    """Test that the b2p service is an instance of B2PService."""
    assert isinstance(client.b2p, B2PService)
...
class TestCallbackProcessing:
    ...
    def test_process_b2p_callback(self, client):
        """Test processing B2Pochi callback payload."""
        payload = {
            "Result": {
                "ResultType": 0,
                "ResultCode": 0,
        ...
        result = client.process_b2p_callback(payload)
        assert result.Result.ResultCode == 0
        assert result.Result.ConversationID == "AG_20240706_2010364430d9bbdaf872"
        assert result.Result.transaction_receipt == "SG632NMUAB"
